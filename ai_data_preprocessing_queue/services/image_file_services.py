...
def scale_too_big_image(image: Image, cutoff_size: int):
    ...
    if _image_size > cutoff_size:
        ...
        scaled_image = scale(image, factor, RESAMPLE)
        return scaled_image
    return image


def scale_too_big_cv2_image(image: np.ndarray, cutoff_size: int):
